...
get_compose_command() {
    ...
    echo "$compose_cmd -f $PROJECT_ROOT/$COMPOSE_FILE"
}
...
check_health_endpoints() {
    ...
    if [[ -n "$gotrue_port" ]]; then
        ...
        print_warning "Production mode detected - health checks require nginx to be running"
        ...
        if curl -s http://localhost/gotrue/health &>/dev/null; then
            ...
            print_info "Cannot check health endpoints externally in production mode"
        fi
    fi
...
check_jwt_secrets() {
    ...
    if [[ -z "$gotrue_secret" ]]; then
        print_error "GOTRUE_JWT_SECRET is not set in .env"
        return 1
    fi

    if [[ -z "$appflowy_secret" ]]; then
        print_error "APPFLOWY_GOTRUE_JWT_SECRET is not set in .env"
        return 1
    fi

    if [[ "$gotrue_secret" != "$appflowy_secret" ]]; then
        print_error "JWT secrets do not match!"
        ...
        print_error "  Fix: Set both to the same value in .env"
        return 1
    fi
...
check_base_urls() {
    ...
    if [[ -z "$base_url" ]]; then
        print_warning "APPFLOWY_BASE_URL is not set"
    else
        print_success "APPFLOWY_BASE_URL: $base_url"
    fi

    if [[ -z "$api_external" ]]; then
        print_warning "API_EXTERNAL_URL is not set"
    else
        print_success "API_EXTERNAL_URL: $api_external"
    fi

    if [[ -z "$gotrue_base" ]]; then
        print_warning "APPFLOWY_GOTRUE_BASE_URL is not set"
    else
...
analyze_service_logs() {
    ...
    for service in $services; do
        ...
        local logs=$($compose_cmd logs --tail=100 $service 2>/dev/null)
        ...
        for pattern in "${patterns[@]}"; do
            if echo "$logs" | grep -i "$pattern" &>/dev/null; then
                if [[ "$found_errors" == "false" ]]; then
                    print_warning "$service: Found potential issues in logs"
                    found_errors=true
                fi
        ...
        if [[ "$found_errors" == "false" ]]; then
            print_success "$service: No obvious errors in recent logs"
        fi
    done
}
...
main() {
    ...
    if [[ "$SKIP_LOGS" != "true" ]]; then
        if [[ "$QUIET" != "true" ]]; then
            print_header "Phase 5: Log Analysis"
        fi
